...
class _EmojiPickerState extends State<EmojiPicker> {
  ...
  Future updateEmojis() async {
    smileyMap =
        await getAvailableEmojis(emojiList.smileys, title: 'smileys') ?? {};
    animalMap =
        await getAvailableEmojis(emojiList.animals, title: 'animals') ?? {};
    foodMap = await getAvailableEmojis(emojiList.foods, title: 'foods') ?? {};
    travelMap =
        await getAvailableEmojis(emojiList.travel, title: 'travel') ?? {};
    activityMap =
        await getAvailableEmojis(emojiList.activities, title: 'activities') ??
            {};
    objectMap =
        await getAvailableEmojis(emojiList.objects, title: 'objects') ?? {};
    symbolMap =
        await getAvailableEmojis(emojiList.symbols, title: 'symbols') ?? {};
    ...
    if (widget.recommendKeywords != null) {
      ...
      recommendedEmojis.sort((a, b) {
        if ((a.tier ?? 0) < (b.tier ?? 0)) {
          ...
          if (a.tier == 1) {
            if ((a.name?.split(" ").length ?? 0) >
                (b.name?.split(" ").length ?? 0)) {
              return -1;
            } else if ((a.name?.split(" ").length ?? 0) <
                (b.name?.split(" ").length ?? 0)) {
              return 1;
            } else {
              return 0;
            ...
            if ((a.numSplitEqualKeyword ?? 0) > (b.numSplitEqualKeyword ?? 0)) {
              return -1;
            } else if ((a.numSplitEqualKeyword ?? 0) <
                (b.numSplitEqualKeyword ?? 0)) {
              return 1;
            } else {
              if ((a.numSplitPartialKeyword ?? 0) >
                  (b.numSplitPartialKeyword ?? 0)) {
                return -1;
              } else if ((a.numSplitPartialKeyword ?? 0) <
                  (b.numSplitPartialKeyword ?? 0)) {
                return 1;
              } else {
                if ((a.name?.split(" ").length ?? 0) <
      ...
      if (recommendedEmojis.length != 0) {
        ...
        for (var i = 0; i < (recommendedPagesNum ?? 0); i++) {
    ...
    for (var i = 0; i < (smileyPagesNum ?? 0); i++) {
    ...
    for (var i = 0; i < (animalPagesNum ?? 0); i++) {
    ...
    for (var i = 0; i < (foodPagesNum ?? 0); i++) {
    ...
    for (var i = 0; i < (travelPagesNum ?? 0); i++) {
    ...
    for (var i = 0; i < (activityPagesNum ?? 0); i++) {
    ...
    for (var i = 0; i < (objectPagesNum ?? 0); i++) {
    ...
    for (var i = 0; i < (symbolPagesNum ?? 0); i++) {
    ...
    for (var i = 0; i < (flagPagesNum ?? 0); i++) {
    ...
    getRecentEmojis().then((_) {
      pages.removeAt(recommendedPagesNum ?? -1);
      pages.insert(recommendedPagesNum ?? -1, recentPage());
      if (mounted) setState(() {});
    });
  }
  ...
  Widget build(BuildContext context) {
    if (loaded) {
      ...
      if (widget.selectedCategory == Category.RECOMMENDED) {
        ...
        pageController = PageController(initialPage: recommendedPagesNum ?? 0);
      } else if (widget.selectedCategory == Category.SMILEYS) {
        pageController = PageController(
            initialPage: (recentPagesNum ?? 0) + (recommendedPagesNum ?? 0));
      } else if (widget.selectedCategory == Category.ANIMALS) {
        pageController = PageController(
            initialPage: (smileyPagesNum ?? 0) +
                (recentPagesNum ?? 0) +
                (recommendedPagesNum ?? 0));
        ...
        children: <Widget>[
          SizedBox(
            ...
            child: PageView(
                ...
                onPageChanged: (index) {
                  if (widget.recommendKeywords != null &&
                      index < (recommendedPagesNum ?? 0)) {
                    widget.selectedCategory = Category.RECOMMENDED;
                  } else if (index <
                      (recentPagesNum ?? 0) + (recommendedPagesNum ?? 0)) {
                    ...
                    child: widget.buttonMode == ButtonMode.MATERIAL
                        ? FlatButton(
                            ...
                            onPressed: () {
                              ...
                              pageController
                                  ?.jumpToPage(0 + (recommendedPagesNum ?? 0));
                            },
                          )
                        : CupertinoButton(
                            pressedOpacity: 0.4,
                    ...
                    child: widget.buttonMode == ButtonMode.MATERIAL
                        ? FlatButton(
                            ...
                            onPressed: () {
                              ...
                              pageController?.jumpToPage((recentPagesNum ?? 0) +
                                  (smileyPagesNum ?? 0) +
                                  (recommendedPagesNum ?? 0));
                            },
                          )
...
class _ProgressPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (selectedCategory == Category.RECOMMENDED) {
      offsetInPages = (pageController?.offset ?? 0) / actualPageWidth;
    } else if (selectedCategory == Category.RECENT) {
      offsetInPages = ((pageController?.offset ?? 0) -
